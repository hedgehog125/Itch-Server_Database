...
Users = ast.literal_eval(" \n".join(Open_File("save/users/Users.txt")))
...
@route('/serverside/user/new', method="POST")
def newUser():
	data = request.json
	try:
		username = data["Name"]
	...
	if confirmPass == password:
		if not username in Users:
			...
			Open_File("save/users/Users.txt",True,str(Users))
			os.makedirs("save/users/" + username)
			Open_File("save/users/" + username + "/Projects.txt",True,str([]))
			Open_File("save/users/" + username + "/Logo.txt",True,"/home/ItchIcon.png")
			return "Ok"
		else:
...
@route('/serverside/user/get/logo/<user>')
def getLogo(user):
	return " \n".join(Open_File("save/users/" + user + "/Logo.txt"))
	

@route('/serverside/user/login', method="POST")
def login():
	data = request.json
...
@route('/serverside/user/listofprojects/<user>')
def listOfProjects(user):
	if user in ast.literal_eval(str(Users)):
		return JSON(ast.literal_eval(" \n".join(Open_File("save/users/" + user + "/Projects.txt"))))
	else:
		return "Error: Unknown user."


@route('/serverside/project/new', method="POST")
def newProject():
	data = request.json
	if "Name" in data and "Pass" in data and "ProjectName" in data:
		if data["Name"] in Users:
			if data["Pass"] == Users[data["Name"]]:
				if not data["ProjectName"] in ast.literal_eval(" \n".join(Open_File("save/users/" + data["Name"] + "/Projects.txt"))):
					projectCount = int(Open_File("save/projects/ProjectCount.txt")[0])
					Open_File("save/projects/ProjectCount.txt",True,str(projectCount + 1))
					Open_File("save/projects/projects/" + str(projectCount + 1) + ".json",True,str([{},{"Owner":data["Name"],"ProjectName":data["ProjectName"]}]))
					...
					Open_File("save/users/" + data["Name"] + "/Projects.txt",True,str(myProjects))
...
@route('/serverside/project/save', method="POST")
def saveProject():
	data = request.json
	if "Name" in data and "Pass" in data and "ProjectId" in data and "ProjectData" in data:
		if data["Name"] in Users:
			projectInfo = ast.literal_eval(" \n".join(Open_File("save/projects/projects/" + data["ProjectId"] + ".json")))[1]
			if data["Name"] == projectInfo["Owner"]:
				if data["Pass"] == Users[projectInfo["Owner"]]:
					Open_File("save/projects/projects/" + data["ProjectId"] + ".json", True, str([data["ProjectData"],{"Owner":projectInfo["Owner"]}]))
					return str(JSON(["Ok"]))
				else:
					return str(JSON(["Error: Your password is incorrect, try logging out and back in again."]))
			else:
...
@route('/serverside/project/get/<id>')
def getProject(id):
	return JSON(ast.literal_eval(" \n".join(Open_File("save/projects/projects/" + id + ".json"))))
